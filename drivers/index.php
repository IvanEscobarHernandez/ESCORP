<!DOCTYPE html>
<html lang="en">
    
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="description" content="Prueba Index">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/sliderInicio.css">
    <title>ESCORP Drivers</title>
    <?php
        require_once "menu.php";
    ?>
</head>

<body>

    <br>
    <div class="imagen_inicio">
        <img src="../img/logoInicio.png" class="imageninicio">
    </div>

    <div class="container-index">

        <div class="contenedor-slider">
            
            <div class="slider" id="slider">
                
                <!-- <div class="slider__section">
                    <div class="word__sectionExtra">
                        <p class="copyExtra">¡ESCORP desea que tengan un año increible!</p><br><br>
                        <p class="copyExtra">Que descubran sus pasiones y se aventuren a seguirlas.</p><br>
                        <p class="copyExtra">Y recuerda, donde sea que estas esten...</p><br>
                        <div style="width: 100%; display:-webkit-box; display:-ms-flexbox;">
                            <h1 class="tituloExtra">¡ESCORP Drivers te lleva! </h1>
                        </div>                             
                        
                    </div>
                    
                    <img src="../img/2022.jpg" alt="" class="slider__img_extra">
                </div> -->

                <div class="slider__section">
                    <div class="word__sectionExtra">
                        <p class="copyExtra">¡ESCORP desea que tengan felices fiestas!</p><br><br>
                        <p class="copyExtra">Y donde quiera que estas sean...</p>
                        <div style="width: 100%; display:-webkit-box; display:-ms-flexbox;">
                            <h1 class="tituloExtra">¡ESCORP Drivers te lleva! </h1>
                        </div>                             
                        
                    </div>
                    
                    <img src="../img/sliderInicioExtra2.jpg" alt="" class="slider__img_extra">
                </div>

                <div class="slider__section">
                    <div class="word__section1">
                        <div style="width: 100%; display:-webkit-box; display:-ms-flexbox;">
                            <h1 class="titulo1">¡Bienvenido a </h1>
                        </div>
                        <div style="width: 100%; display:-webkit-box; display:-ms-flexbox;">
                            <h1 class="titulo1">ESCORP </h1> <h1 class="italicas"> Drivers!</h1>
                        </div>
                        <br><br>
                        <div style="width: 100%; display:-webkit-box; display:-ms-flexbox;">
                            <b><h7 class="copy1">!Una forma mas SEGURA de viajar! </h7></b>
                        </div>
                    </div>
                    
                    <img src="../img/sliderInicio1.jpg" alt="" class="slider__img">
                </div>

                <div class="slider__section">
                    <div class="word__section2">                             
                        <p class="copy2"> ¡Donde nuestro activo más valioso es tu CONFIANZA!</p>
                    </div>
                    
                    <img src="../img/sliderInicio2.jpg" alt="" class="slider__img">
                </div>

                <div class="slider__section">

                    <div class="word__section3">
                        <h1 class="titulo3">¿Cansado de lo mismo?</h1>
                        <p class="copy3"> Cargos extra, cancelaciones, esperar que acepten tu viaje, o sentirte inseguro todo el tiempo...</p>
                    </div>

                    <img src="../img/sliderInicio3.jpg" alt="" class="slider__img2">
                </div>

                <div class="slider__section">

                    <div class="word__section4">
                        <h1 class="titulo4">¡No lo pienses más!</h1>
                        <p class="copy4"> ¡Somos tu mejor opción!</p>
                    </div>

                    <img src="../img/sliderInicio4.jpg" alt="" class="slider__img">
                </div>

            </div>
        
            <div class="slider__btn slider__btn--right" id="btn-right"> > </div>
            <div class="slider__btn slider__btn--left" id="btn-left"> < </div>
            

        </div>

        <!-- <footer>
            <div class="section_footer">
                <div class="n_footer">
                    2022 Website by ESCORP Technologies | All rights reserved.
                </div>
            </div>
        </footer> -->
        <section class="footer">
            <p>2020-2022 Website by ESCORP Technologies | All rights reserved.</p>
        </section>
        
    </div>

    

</body>

<script src="../js/sliderInicio.js"></script>

</html>